...
class EventoModel extends ModelAbstract
{
	...
	function getData($id=null,$type=null){
		if($type == null){
			if($id === null){
				$query = "SELECT * FROM evento WHERE aberto = 1 and (ativo = 's' AND DATEDIFF(CURDATE(), data_limite) <= 0)";
			}
			else{
				$query = "SELECT * FROM evento WHERE id_evento = '$id' and aberto = 1 and (ativo = 's' AND DATEDIFF(CURDATE(), data_limite) <= 0)";
			}
		}else{
			if($id === null){
				$query = "SELECT * FROM evento WHERE ativo = 'n' OR DATEDIFF(CURDATE(), data_limite) > 0";
			}
			else{
				$query = "SELECT * FROM evento WHERE id_evento = '$id' and (ativo = 'n' OR DATEDIFF(CURDATE(), data_limite) > 0)";
			}
		}
